...
data_dir <- file.path(analysis_dir, "data/combp_smoking")
...
get_probe_annotations <- function(label) {
  save_name <- file.path(
    save_dir,
    paste0("linear_model_", label, "_smoking"),
...
combp_wrapper <- function(label) {
  ...
  write.csv(
    dmr_df,
    file = file.path(save_dir, "comb-p-DMRs_smoking", paste0(label, "_DMRs.csv")),
    row.names = FALSE
  )
}
```
